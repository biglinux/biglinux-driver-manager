...
if uname -r | grep xanmod.*lts >/dev/null; then current="linux-xanmod-lts"; elif uname -r | grep xanmod >/dev/null; then current="linux-xanmod";  else current="linux$major$minor"; fi
IFS=$'\n'
